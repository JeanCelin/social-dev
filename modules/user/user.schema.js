...
export const signupSchema = Joi.object ({
  ...
  lastName: Joi.string().required().max(50).message('O campo "sobrenoome" pode ter no máximo {{#limit}} caracteres.'),
  user: Joi.string().required().max(30).message('O campo "usuário" pode ter no máximo {{#limit}} caracteres.'),
  email: Joi.string().email({ tlds: { allow: false } }).required().max(100).message('O campo "e-mail" pode ter no máximo {{#limit}} caracteres.'),
